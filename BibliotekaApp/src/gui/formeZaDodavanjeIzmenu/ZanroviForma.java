...
public class ZanroviForma extends JFrame {
	private static final long serialVersionUID = 6942666547551007478L;

	private JLabel lblID = new JLabel("ID");
	private JTextField txtID = new JTextField(20);
	private JLabel lblOznaka = new JLabel("Naslov");
	...
	public ZanroviForma(BibliotekaApp biblioteka, Zanr zanr, DefaultTableModel tableModel, JTable zanroviTabela) {
		...
		btnOK.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(validacija()) {
					...
					if(zanr == null) {
						zanr = new Zanr(id, oznaka, opis, false);
	...
	private boolean validacija() {
		...
		if(txtID.getText().trim().equals("")) {
			...
			int id = Integer.parseInt(txtID.getText().trim());
			Zanr pronadjen = biblioteka.pronadjiZanr(id);
			if(pronadjen != null) {
	...
	private Object[] kreirajRedTabele(Zanr zanr) {
	...
	private void refresh(int red, Zanr zanr) {
			if(red >= 0) {
